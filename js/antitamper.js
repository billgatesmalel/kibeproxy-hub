...
// Admin page: fully excluded
// Auth page: fully excluded (login/logout/signup free)
// No location restrictions - accessible from anywhere
// No phone restrictions - multiple accounts per device allowed

(function() {
  'use strict';

  // ── FULL BYPASS for admin and auth pages ─────────────────────
  const path = window.location.pathname;
  if (path.includes('admin.html') || path.includes('auth.html')) return;
  ...
  function checkDevTools() {
    ...
    if ((w || h) && !devToolsOpen) {
      ...
      devToolsOpen = false; warningShown = false;
      const o = document.getElementById('tamper-overlay');
      if (o) o.remove();
      document.querySelectorAll('button,input,select').forEach(el => el.disabled = false);
    }
  }

  // ── KEYBOARD SHORTCUTS (store page, production only) ─────────
  document.addEventListener('keydown', e => {
    ...
    if (
      ...
      e.preventDefault();
      showTamperWarning('Keyboard shortcut blocked.');
      return false;
    }
  });

  // ── RIGHT CLICK (store page, production only) ─────────────────
  document.addEventListener('contextmenu', e => {
    if (IS_PROD && IS_STORE) { e.preventDefault(); return false; }
  });

  // ── PRICE INTEGRITY (store page only) ────────────────────────
  ...
  const observer = new MutationObserver(m => {
    m.forEach(mut => {
      mut.addedNodes.forEach(n => {
        if (n.nodeName === 'SCRIPT' && n.src &&
            !n.src.includes('supabase') &&
            !n.src.includes('jsdelivr') &&
            !n.src.includes('fonts') &&
            !n.src.includes('cloudflare') &&
            !n.src.includes('antitamper')) {
  ...
  // ── CONSOLE OVERRIDE (production, store only) ─────────────────
  if (IS_PROD && IS_STORE) {
    const noop = () => {};
    ['log','warn','error','info','debug'].forEach(k => window.console[k] = noop);
  }
